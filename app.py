...
class Show(db.Model):
    ...
    def __repr__(self):
        return '<Show: Venue: {} Artist: {}>'.format(self.venue_id, self.artist_id)
...
def show_venue(venue_id):
  ...
  upcomning_shows_query = db.session.query(Show).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time > datetime.now()).all()
  upcoming_shows = []

  past_shows_query = db.session.query(Show).join(Artist).filter(Show.venue_id==venue_id).filter(Show.start_time < datetime.now()).all()
  past_shows = []

  for show in upcomning_shows_query:
...
def delete_venue(venue_id):
  ...
  try:
    ...
    db.session.close()
  if error:
    flash('Venue with venue_id' + request.form['venue_id'] + 'could not be deleted!')
  if not error:
    flash('Venue with venue_id' + request.form['venue_id'] + 'was deleted!')
  return render_template('pages/home.html')
...
def edit_artist_submission(artist_id):
  ...
  try:
    ...
    db.session.close()
  if error:
    flash('Artist could not be updated / changed.')
  if not error:
...
def create_artist_submission():
  ...
  try:
    ...
    artist = Artist(name=name, city=city, genres=genres, state=state, phone=phone, facebook_link=facebook_link, image_link=image_link, website=website, seeking_venue=seeking_venue, seeking_description=seeking_description)
...
def create_show_submission():
  ...
  try:
    ...
    db.session.close()
  if error:
    flash("Show not created successfully!")
  if not error:
